...
namespace NopBrasil.Plugin.Shipping.Correios.Service
{
    public class CorreiosService : ICorreiosService
    {
        ...
        {
            decimal declaredValue = GetConvertedRateFromPrimaryCurrency(shippingOptionRequest.Items.Sum(item => (item.ShoppingCartItem.Quantity * item.ShoppingCartItem.Product.Price)));
